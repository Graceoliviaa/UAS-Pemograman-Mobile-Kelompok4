📱 WGS Project Management System - Kelompok 4

Sistem Informasi Manajemen Proyek, Klien, Tim, dan Dokumen internal yang dirancang khusus untuk memenuhi tugas Ujian Akhir Semester mata kuliah Pemrograman Mobile 1 di Universitas Teknologi Bandung. Aplikasi ini dibangun menggunakan platform Android Native dengan bahasa pemrograman Kotlin dan memanfaatkan Room Database untuk penyimpanan data lokal secara persisten.

---

👥 Daftar Anggota Kelompok & Peran

1. Grace Olivia Hutagalung - 24552011114 - Lead Android Developer (Foundation DB, Entity/DAO, Project Module: MainActivity, ProjectListActivity, ProjectFormActivity, ProjectDetailActivity)

2. Revana Augustin Mustika Rauf - 24552011343 - Android Developer (Client & Team Module: ClientListActivity, ClientFormActivity, ProjectTeamActivity, AssignMemberActivity)

3. Ai Siti Qomariah - 24552011276 - Android Developer & QA Tester (Document Module & Testing Integration: ProjectDocumentActivity, DocumentFormActivity, Integration Testing, Employee Dummy Data Provider)

---
📄Laporan OOAD
https://drive.google.com/drive/folders/152d2wxIoaNeL1I54BNa2i5s0wGwm6LG-?usp=drive_link


---
🎥 Video Penjelasan & Demo Aplikasi
https://youtu.be/GBFzcSq_FbI?si=JB88CGJ0S8voYHh3

---

📸 Screenshots Aplikasi
<img width="250" alt="1" src="https://github.com/user-attachments/assets/4d3d3922-8429-46f1-9d3d-639c6f768c44" />
<img width="250" alt="2" src="https://github.com/user-attachments/assets/2704d0cd-3c72-4791-936b-79f656074a5f" />

---

🚀 Fitur Utama Aplikasi
1. Autentikasi Hak Akses: Log in aman menggunakan akun Project Manager.
2. Manajemen Proyek (CRUD): Menambah, melihat detail, memperbarui informasi, dan memantau status proyek perusahaan.
3. Manajemen Klien: Integrasi data klien, contact person (PIC), email, serta alamat perusahaan yang terhubung langsung ke proyek terkait.
4. Alokasi Tim Proyek: Menugaskan karyawan ke dalam tim proyek spesifik lengkap dengan penentuan role kerja.
5. Arsip Dokumen Proyek: Pengunggahan berkas digital pendukung proyek lengkap dengan pencatatan tanggal otomatis.

---

🛠️ Cara Menjalankan / Cloning Proyek
Ikuti langkah berikut untuk menjalankan proyek ini di Android Studio lokal Anda:

1. Clone Repository:
   git clone https://github.com/Graceoliviaa/UAS-Pemograman-Mobile-Kelompok4.git
